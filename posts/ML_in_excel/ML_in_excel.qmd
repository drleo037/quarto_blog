...
```{r scatterplot of simple ranger model, warning=FALSE, message=FALSE}
...
1.  Fitted a few models: one random forest of decision trees for classification and one for regression, and an extra linear model for regression.
...
trees_df_iris_xgboost_reg <- tidypredict::tidypredict_sql(model_reg_xgboost, dbplyr::simulate_dbi()) %>%
  tibble::enframe(name = NULL, value = "instruction") %>%
  mutate(instruction = unlist(instruction))
...
tribble(
  ...
  "LAK", "classification", "An example of a classification of Iris Species using random forest",
  "LAK", "classificationRules", "One row per tree in the classification random forest.  Easy-to-read SQL logic format",
  "LAK", "lm", "An example of a regression to predict Petal Length using a linear model",
  "LAK", "lmRules", "The internal equations in the linear model in an easy-to-read format",
  "LAK", "regression", "An example of predicting Petal Length a regression random forest",
  "LAK", "regressionRules", "One row per tree in the regression random forest.  Easy-to-read SQL logic format"
) %>%
