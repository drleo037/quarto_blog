...
library(osmdata) # this package mapes getting small datasets from OSM simple and quick
...
The OSM dataset has lots of features (including allotments, and other potentially useful categories)
...
```{r}
# sf::st_layers(system.file("C:/Users/leoki/DATA/OS/OS_Open_Zoomstack/OS_Open_Zoomstack.gpkg", "sf"))
```
...
```{r read allotments in Reading from }
...
# the followinf code just converts the coords of the focus_boundary into a
...
```{r crosstalk interactivity between maps and tables of allotments}
...
sd <- SharedData$new(focus_allotments$osm_polygons %>%
                       mutate(area = sf::st_area(geometry)) %>%
                       remove_rownames() %>%
                       select(name, landuse, operator, contact = `contact:facebook`, area)
)

# Create a filter input
crosstalk::filter_slider("area", "Allotment Area (m2)",
                         sd,
                         column=~as.numeric(area),
                         step=100, width=250)
