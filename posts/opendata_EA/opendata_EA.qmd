...
We can ask the API to return historic values recorded for measurements using the */readings* endpoint. In the code below I gather time series for *gauge levels* from 21 EA *stations* along the *River Thames*. I'm also protecting myself from any potential confusion in units by only selecting measures with units mASD[^5].
